...
public class DataJpaMenuRepository {
    ...
    public List<Menu> getByDate(LocalDate date) {
        logger.info(String.format("Get menu by date=%s", date.toString()));
    ...
    public List<MenuVotingResultsTo> getMenuVotingResults(LocalDate date) {
        logger.info(String.format("Get menu voting results by date", date.toString()));
        return crudMenuRepository.getMenuVotingResults(date);
    }
